...
import type { CostCenter, PaymentMode, Role } from "@prisma/client";
import { prisma } from "./prisma";
import { NON_EXPENSE_LABELS } from "../../prisma/seed-data";

/**
 * Migration of the three legacy workbooks, driven either by the CLI
 * (scripts/import-legacy.ts) or by the founder's Import screen.
 *
 * The important correction it makes: cash the founder handed to Hardikbhai
 * (warehouse float) or Maulikbhai (head-office float) was booked as an *expense*
 * in the founder's workbook, and the same money was then booked again by the
 * WH/HO teams when they spent it. Those rows become float ISSUE transfers, so
 * group totals stop double-counting.
 */
export type LegacyFiles = { founder: Buffer; wh: Buffer; ho: Buffer };
...
async function load(buf: Buffer, which: string) {
  try {
    const wb = new ExcelJS.Workbook();
    // ExcelJS types the reader as accepting a Node stream, but a Buffer works and
    // is what an upload hands us.
    await wb.xlsx.load(buf as unknown as ArrayBuffer);
    return wb;
  } catch {
    throw new Error(`The ${which} file could not be read as an .xlsx workbook.`);
  }
}

function sheet(wb: ExcelJS.Workbook, name: string, which: string) {
  const ws = wb.getWorksheet(name);
  if (!ws) {
    throw new Error(
      `The ${which} workbook has no "${name}" sheet. Found: ${wb.worksheets.map((w) => w.name).join(", ")}`,
    );
  }
  return ws;
}
...
/**
 * Who migrated rows are attributed to.
 *
 * Historical rows predate anyone having an account, so they are credited to whoever
 * currently holds that role. When a team has no account yet — the usual case right
 * after first-run setup — a placeholder is created to carry authorship. It is inactive
 * and has no usable password, so it cannot be signed into; once the real warehouse or
 * head-office account is added on the People screen, new entries are theirs.
 */
async function authorFor(role: Role, preferId?: string) {
  if (preferId) {
    const preferred = await prisma.user.findUnique({ where: { id: preferId } });
    if (preferred && preferred.role === role) return preferred;
  }
  ...
  const label = role === "WH" ? "Warehouse team" : role === "HO" ? "Head office team" : "Founder";
  return prisma.user.create({
    data: {
      name: `${label} (historical)`,
      email: `${role.toLowerCase()}.historical@invalid.local`,
      role,
      costCenter: role === "FOUNDER" ? null : role,
      // no password hash can ever match this, and the account is inactive anyway
      passwordHash: "!",
      active: false,
    },
  });
...
export async function runImport(files: LegacyFiles, actorId?: string): Promise<ImportResult> {
  aliasMap = new Map();
  unmapped.clear();
  await loadCategories();

  const founder = await authorFor("FOUNDER", actorId);
  const teamUser: Record<string, string> = {
    WH: (await authorFor("WH")).id,
    HO: (await authorFor("HO")).id,
  };

  await prisma.expense.deleteMany({ where: { legacyRef: { not: null } } });
  await prisma.cashTxn.deleteMany({ where: { legacyRef: { not: null } } });

  const wbF = await load(files.founder, "founder");
  const wbW = await load(files.wh, "warehouse");
  const wbH = await load(files.ho, "head office");
  ...
  const ledger = readSheet(sheet(wbF, "💵 Cash Ledger", "founder"), 5,
  ...
  const inflow = readSheet(sheet(wbF, "💰 Cash Inflow", "founder"), 5,
  ...
  const draw = readSheet(sheet(wbF, "👤 Director Drawings", "founder"), 4,
  ...
  const hist = readSheet(sheet(wbF, "🗂 Historical FY24-25", "founder"), 4,
  ...
  for (const [cc, wb] of [["WH", wbW], ["HO", wbH]] as const) {
    const ws = sheet(wb, "💰 Cash Book", cc === "WH" ? "warehouse" : "head office");
  ...
  return {
    ...
    stats: Object.fromEntries(Object.entries(stats).sort()),
    unmapped: [...unmapped].sort((a, b) => b[1] - a[1]).slice(0, 25).map(([label, count]) => ({ label, count })),
  };
}
